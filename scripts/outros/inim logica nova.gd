...
func atacar(alvo):
	if $Timer.is_stopped():
		#alvo.saude -= randi_range(-10, -40)
		$Timer.wait_time = randf_range(1.5, 4.0)
		$Timer.start()
		print("atacando...")

func logica_raycast(delta):
	if !stun:
		var colisor = raycast.get_collider()
		print(ultima_pos)
		if global_position.distance_to(pers.global_position) < 20:
			raycast.show()
			raycast.position = position
			raycast.target_position = raycast.to_local(pers.global_position)
			if colisor != null and colisor.is_in_group("Player") and colisor.global_position.distance_to(global_position) < 50:
				idle = false
				ultima_pos = colisor.global_position
				movimentar_navimesh(pers.global_position)
				#prints("colisor", colisor)
				#prints("ultima_pos", ultima_pos)
		elif colisor == null or (!colisor.is_in_group("Player") and ultima_pos):
